...
app.get("/filter",(req,res)=>{
  ...
  const filterd=jokes.filter((joke)=>joke.jokeType===type);
  const randomIndex=Math.floor((Math.random())*filterd.length);
  res.json(filterd[randomIndex]);
})
//4. POST a new joke
...
app.delete("/deleteall",(req,res)=>{
    if(req.query.key == masterKey )
    {
      ...
      res.status(404).json({
        error:`You are npt authorised to perform this action.`});
    }
})
